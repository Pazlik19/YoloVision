...
import numpy as np
import os
def get_real_coords(u, v, H, W_p=640, H_p=480):
    """
    ...
    W_p, H_p - ФАКТИЧЕСКОЕ разрешение кадра, в котором найдены (u, v). Должно совпадать
    с размером кадра на сервере детекции (frame.shape), иначе оптический центр и GSD
    посчитаются неверно и центрированный объект получит ненулевые координаты.
    """
    # Параметры сенсора Sony IMX219 (физический размер используемой области матрицы).
    # Не зависят от разрешения трансляции — это поле зрения (FOV), а не пиксели.
