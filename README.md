...
| `Models/` | Веса моделей (`.pt`, `.pth`, `.engine`) — **не в git**; `convertResnetTensor.py` — сборка TensorRT-движка ResNet на Jetson |
...
- **`map_view.py`** — HTML-страница карты поля (эндпоинт `/map`): canvas с сеткой в мм, зоной сканирования камеры (FOV) и объектами сцены; данные тянутся опросом `/data`.
- **`convertResnetTensor.py`** — конвертация ResNet18 (`.pth`) в TensorRT-движок (`best_resnet_trt.pth`, FP16) через `torch2trt`; запускать **только на Jetson**. Сервер подхватывает движок автоматически, иначе откатывается на `.pth`.
...
> и подписями) — в [`docs/diagrams.md`](docs/diagrams.md).
